function [J, f] = AutoDiffJacobian(func, x, p, method, varargin)
...
if strcmpi(method, 'AutoDiff')
    [J, f] = AutoDiffJacobianAutoDiff(func, x, p);
elseif strcmpi(method, 'FiniteDiff')
    [J, f] = AutoDiffJacobianFiniteDiff(func, x, p, 1:length(x), args.GradEpsilons);
elseif strcmpi(method, 'VerifiedAutoDiff')
    [J, f] = AutoDiffJacobianAutoDiff(func, x, p);
    [J2, ~] = AutoDiffJacobianFiniteDiff(func, x, p, 1:numel(x), args.GradEpsilons);
